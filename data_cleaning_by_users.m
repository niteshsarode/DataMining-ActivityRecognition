    ...
    if contains(folders_myo(j).name,"user")
         ...
         for k=1:length(files)
              if contains(files(k).name,"EMG")
                  ...
                  tf_data = csvread("groundTruth/user9/fork/"+files_gt(1).name);
                  start_sample = {};
                  end_sample = {};
                  ...
                  for l=1:length(tf_data)
                      for t=cursor:e_t(l)
                          if t < s_t(l)
                              emg_e_mat = [emg_e_mat;f_data(t,:),0]; % 0 for non-eating
                          elseif t >= s_t(l) && t <= e_t(l)
                              emg_e_mat = [emg_e_mat;f_data(t,:),1]; % 1 for eating
                          end
                      end
                      cursor = floor(e_t(l));
                  end
              ...
              if contains(files(k).name,"IMU")
                  ...
                  for l=1:length(tf_data)
                      for t=cursor:e_t(l)
                          if t < s_t(l)
                              imu_e_mat = [imu_e_mat;f_data(t,:),0]; % 0 for non-eating
                          elseif t >= s_t(l) && t <= e_t(l)
                              imu_e_mat = [imu_e_mat;f_data(t,:),1]; % 1 for eating
                          end
                      end
                      cursor = floor(e_t(l));
                  end
